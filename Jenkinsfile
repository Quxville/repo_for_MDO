    ...
    stages {
        ...
        stage('Build') {
            steps {
                ...
                sh 'docker build -t irssibld -f building/Dockerfile .'
        ...
        stage('Test') {
            steps {
                sh 'docker build -t irssitst -f testing/Dockerfile .'
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'docker container exec deploybuffer sh -c "apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y libglib2.0 libutf8proc-dev" >> log-deploy.txt'
